...
| 04 | apex-refresh | **passed 2026-09-17** (live Chrome, post-import) — three real `REFRESH_FIXTURE` clicks on the imported, corrected page 409: `apexafterrefresh` 3/3 (was 0), a new Alpine root each time with the old one disconnected, one instance and one click handler after every swap, `Alpine.start()` never re-called, item synced both ways. The evaluee half (exemplary, 2026-09-16) is runs/2026-09-16/04-apex-refresh-current.md — runs/2026-09-17/04-apex-refresh-postimport.md |
| 05 | source-persistence | **passed 2026-09-17** (live Chrome, post-import) — clauses (a)+(b) graded clean on 2026-09-16; (c)+(d) now measured on this repo's own prototype → source → `sync-static.sh` → import → reload path: p1906 MapLibre attribution 2.57:1 → **12.25:1** and the restated `--a-palette-*` chain resolving to package values, with **0 injected `<style>` tags**. Substitution on record: the evaluee's own hero-card CSS lived only in a discarded worktree and was never committed, so it could not be the imported artifact — runs/2026-09-17/05-source-persistence-postimport.md |
...
| 11 | dark-package-coverage | **passed 2026-09-17** (live Chrome, post-import re-sweep; the verdict is on the package) — 24 pages at 1440 + 4 at 375 + the dialog iframe: **7 failures, 0 package-caused**, both groups proven UT/APEX-owned by bare-Iris A/B (byte-identical with and without the package class). All four 2026-09-16 defects measured fixed: IG row selection **1.06:1 → 8.17:1**, faceted search 2.39:1 → 0 failures, MapLibre 2.57:1 → 12.25:1, and JET charts — unobservable before an import existed — **25/25 SVG text nodes pass**, worst 4.86:1. Residual coverage (98 pages, 1024/768, other selection states) carried forward in the promoted finding. Prior FAIL: runs/2026-09-16/11-dark-package-coverage-current.md — runs/2026-09-17/11-dark-package-coverage-postimport.md |
...
**2026-09-17 post-import addendum.** Three of those verdicts were blocked by one environmental fact — the
running app served the build already in the database, and no session in that round was permitted to import.
The user ran `scripts/sync-static.sh` → `scripts/apex-validate.sh` → `scripts/apex-import.sh` on 2026-09-17,
and this session re-took only the measurements the round could not take (logs in `runs/2026-09-17/`, all
labelled as grader measurements rather than fresh evaluee runs):

| # | Was | Now | What actually changed |
|---|---|---|---|
| 04 | UNVERIFIED | **PASS** | the fixture can refresh: `apexafterrefresh` 3/3, new root each time, one handler, `Alpine.start()` never re-called |
| 05 | UNVERIFIED | **PASS** (substitution on record) | reload-without-injection measured on this repo's own source→sync→import path; the evaluee's own CSS was never committed and could not be the artifact |
| 11 | **FAIL** | **PASS** | re-sweep of the fixed, imported package: 0 package-caused failures; the 1.06:1 selection defect measures 8.17:1 and JET measures 25/25 |

The 2026-09-16 rows above are left exactly as graded — they were correct about the build that existed then.
